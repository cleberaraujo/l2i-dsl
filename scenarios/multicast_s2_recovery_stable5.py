...
def compute_recovery(
    *,
    phase_plan: Any,
    samples_B: List[Tuple[int, float, bool]],
    samples_C: List[Tuple[int, float, bool]],
    event_name: str,
    bin_ms: Optional[int] = None,
    interval_ms: Optional[int] = None,
    rtt_limit_ms: float = 0.0,
    delivery_limit: float = 0.0,
    stable_k_bins: int = 3,
    **_compat: Any,
) -> Dict[str, Any]:
    """
    Calcula série temporal por bins e recovery após o evento.

    Definições:
    - A contagem do recovery é medida a partir de event_end_ms (fim do evento).
    - time_to_first_conformance_ms: primeiro bin >= event_end_ms com intent_ok==True.
    - time_to_stable_conformance_ms: primeiro instante >= event_end_ms em que intent_ok
      fica True por K bins consecutivos (stable_k_bins).
    """
    # --- normaliza phase_plan (aceita dict ou list) ---
    phases: List[Dict[str, Any]] = []
    duration_ms = 0

    if isinstance(phase_plan, dict):
        duration_ms = int(phase_plan.get("duration_ms") or phase_plan.get("total_ms") or 0)
        raw_phases = phase_plan.get("phases") or phase_plan.get("phase_plan") or []
        if isinstance(raw_phases, list):
            phases = [p for p in raw_phases if isinstance(p, dict)]
    elif isinstance(phase_plan, list):
        phases = [p for p in phase_plan if isinstance(p, dict)]
        duration_ms = int(max((int(p.get("end_ms", 0)) for p in phases), default=0))

    def _is_event(p: Dict[str, Any]) -> bool:
        if str(p.get("name", "")).lower() == "event":
            return True
        if p.get("event") and str(p.get("event")).lower() == str(event_name).lower():
            return True
        if event_name and str(p.get("name", "")).lower() == str(event_name).lower():
            return True
        return False

    ev = next((p for p in phases if _is_event(p)), {}) or {}
    ev_s = int(ev.get("start_ms", 0))
    ev_e = int(ev.get("end_ms", 0))

    # duração total do experimento (ms)
    if duration_ms <= 0:
        max_t = 0
        if samples_B:
            max_t = max(max_t, int(max(t for t, _, _ in samples_B)))
        if samples_C:
            max_t = max(max_t, int(max(t for t, _, _ in samples_C)))
        duration_ms = max_t

    # compat: alguns chamadores antigos usam interval_ms em vez de bin_ms
    if bin_ms is None:
        bin_ms = interval_ms
    if bin_ms is None:
        bin_ms = 500  # fallback seguro (não deveria acontecer se main passa --recovery-bin-ms)

    w = max(1, int(bin_ms))

    # Normaliza amostras (robustez): aceita formatos (t,rtt,ok) ou dicts {"t_ms","rtt_ms",...}
    def _norm_samples(samples: Any) -> List[Tuple[int, float, bool]]:
        ...
        for it in samples:
            try:
                if isinstance(it, dict):
                    t = it.get("t_ms", it.get("t", it.get("ts_ms", it.get("ts", 0))))
                    rtt = it.get("rtt_ms", it.get("rtt", it.get("rtt", 0.0)))
                    ok = it.get("ok", it.get("success", True))
                elif isinstance(it, (list, tuple)) and len(it) >= 2:
                    t = it[0]
                    rtt = it[1]
                    ok = it[2] if len(it) >= 3 else True
                else:
                    continue
                t_i = int(float(t))  # aceita str/float/int
                r_f = float(rtt)
                out.append((t_i, r_f, bool(ok)))
            except Exception:
                continue
        return out

    samples_B = _norm_samples(samples_B)
    samples_C = _norm_samples(samples_C)

    n_bins = max(1, int((duration_ms + w - 1) // w))
    bins = [(i * w, min((i + 1) * w, duration_ms)) for i in range(n_bins)]

    def _bin_stats(samples: List[Tuple[int, float, bool]], s: int, e: int) -> Tuple[Optional[float], float]:
        rtts = [rtt for (t, rtt, ok) in samples if s <= t < e]
        oks = [ok for (t, rtt, ok) in samples if s <= t < e]
        if not rtts:
            return None, 0.0
        rtts.sort()
        p99 = rtts[int(round(0.99 * (len(rtts) - 1)))]
        delivery = sum(1 for x in oks if x) / len(oks) if oks else 0.0
        return float(p99), float(delivery)

    series: List[Dict[str, Any]] = []

    # monta bins
    for (s, e) in bins:
        p99B, delB = _bin_stats(samples_B, s, e)
        p99C, delC = _bin_stats(samples_C, s, e)

        ok_rtt = True
        if p99B is not None and p99B > rtt_limit_ms:
            ok_rtt = False
        if p99C is not None and p99C > rtt_limit_ms:
            ok_rtt = False

        ok_del = (delB >= delivery_limit) and (delC >= delivery_limit)
        intent_ok = bool(ok_rtt and ok_del)
        ...
        series.append({
            ...
            "rtt_p99_ms_B": p99B,
            "rtt_p99_ms_C": p99C,
            "delivery_ratio_B": round(float(delB), 6),
            "delivery_ratio_C": round(float(delC), 6),
            "intent_ok": intent_ok,
        })

    # recovery: a partir do fim do evento (event_end_ms)
    # time_to_first_conformance_ms: primeiro bin com intent_ok True após event_end_ms
    t_first: Optional[int] = None
    for b in series:
        if b["start_ms"] >= ev_e and b["intent_ok"]:
            t_first = int(b["start_ms"] - ev_e)
            break

    # time_to_stable_conformance_ms: primeiro instante com K bins consecutivos True após event_end_ms
    K = max(1, int(stable_k_bins))
    t_stable: Optional[int] = None
    for i in range(len(series)):
        if series[i]["start_ms"] < ev_e:
            continue
        ok_run = True
        for j in range(K):
            if i + j >= len(series) or not series[i + j]["intent_ok"]:
                ok_run = False
                break
        if ok_run:
            t_stable = int(series[i]["start_ms"] - ev_e)
            break
    ...
    return {
        ...
        "event_start_ms": ev_s,
        "event_end_ms": ev_e,
        "measured_from": "event_end_ms",
        "stable_k_bins": K,
        "bin_ms": w,
        "rtt_limit_ms": float(rtt_limit_ms),
        "delivery_limit": float(delivery_limit),
        "time_to_first_conformance_ms": t_first,
        # alias compatível
        "time_to_conformance_ms": t_first,
        "time_to_stable_conformance_ms": t_stable,
        "series": series,
    }
def main() -> None:
    ...
    ap.add_argument("--stable-k-bins", type=int, default=3,
                    help="K bins consecutivos para definir recovery estável (default 3)")
    ...
    s2["recovery"] = compute_recovery(
        ...
        stable_k_bins=int(args.stable_k_bins),
    )
