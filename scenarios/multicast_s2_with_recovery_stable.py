...
def compute_recovery(
    ...
    """
    Constrói uma série temporal (bins) e deriva métricas de recovery.

    Definições (para evitar ambiguidade metodológica):
    - O evento ocorre no intervalo [event_start_ms, event_end_ms].
    - O recovery é medido **a partir de event_end_ms** (fim do evento), não do início.
      Assim, se o primeiro bin pós-evento começa em event_end_ms e já está conforme,
      o tempo pode ser 0 ms (isso é intencional e defensável).
    - time_to_first_conformance_ms: primeiro bin pós-evento em que intent_ok == true.
    - time_to_stable_conformance_ms: primeiro instante pós-evento em que intent_ok == true
      por K bins consecutivos (K = stable_k_bins). Evita "recuperação por sorte".

    samples_*: lista de tuplas (t_ms, rtt_ms, ok_delivery).
    interval_ms é o intervalo nominal de amostragem (auditoria); bin_ms é o tamanho das janelas na série.
    """
    ev = phase_plan.get("event") or {}
    ...
    series = []
    for (s, e) in bins:
        p99_B, del_B = _bin_stats(samples_B, s, e)
        p99_C, del_C = _bin_stats(samples_C, s, e)

        ok_rtt = True
        if p99_B is not None and p99_B > rtt_limit_ms:
            ok_rtt = False
        if p99_C is not None and p99_C > rtt_limit_ms:
            ok_rtt = False

        ok_delivery = (del_B >= delivery_limit) and (del_C >= delivery_limit)

        intent_ok = bool(ok_rtt and ok_delivery)

        series.append({
            "start_ms": s,
            "end_ms": e,
            "rtt_p99_ms_B": None if p99_B is None else round(p99_B, 3),
            "rtt_p99_ms_C": None if p99_C is None else round(p99_C, 3),
            "delivery_ratio_B": round(del_B, 3),
            "delivery_ratio_C": round(del_C, 3),
            "intent_ok": intent_ok,
        })

    # pós-evento (start >= event_end_ms)
    post = [b for b in series if int(b["start_ms"]) >= ev_e]

    # 1) primeira conformidade
    t_first: Optional[int] = None
    for b in post:
        if bool(b.get("intent_ok")):
            t_first = int(b["start_ms"]) - ev_e
            if t_first < 0:
                t_first = 0
            break

    # 2) conformidade estável por K bins consecutivos
    ...
    for i in range(len(post)):
        ...
        if len(window) < k:
            break
        if all(bool(x.get("intent_ok")) for x in window):
            t_stable = int(post[i]["start_ms"]) - ev_e
            if t_stable < 0:
                t_stable = 0
            break
    ...
    return {
        ...
        "event_start_ms": ev_s,
        "event_end_ms": ev_e,
        "time_reference": "event_end_ms",
        "stable_k_bins": k,
        "sampling_interval_ms": int(interval_ms),
        "bin_ms": w,
