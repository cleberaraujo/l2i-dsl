...
Operational S2 integration remains deferred until a canonical S2 entry point explicitly
...
## Phase 4R/4RB S2/RQ4 synthetic assurance harness

`scenarios.multidomain_s2` is only the synthetic assurance qualification
harness and is not the operational S2 entry point. It exposes
...
Qualification-only invocations use no scientific slot and materialize
`plan_binding.applicable=false`. Planned invocations bind CLI backend, profile,
treatment and repository provenance to the validated slot. The harness accepts
only `backend=mock`; `backend=real` fails before results-directory creation.

The harness persists an exclusive manifest and durable `RUNNING` attempt
